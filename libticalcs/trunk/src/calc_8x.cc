...
static int		execute		(CalcHandle* handle, VarEntry *ve, const char* args)
{
	...
	PAUSE(200);
	int ret = send_key(handle, KEY83_Quit);
	if (!ret)
	{
...
static int		recv_screen_80	(CalcHandle* handle, CalcScreenCoord* sc, uint8_t** bitmap)
{
	*bitmap = (uint8_t *)ticalcs_alloc_screen(TI80_COLS * TI80_ROWS / 8);
	...
	int ret = ti80_send_SCR(handle);
	if (!ret)
	{
		ret = ti80_recv_ACK(handle, nullptr);
		if (!ret)
		{
			uint16_t max_cnt;
			...
			{
				int i = 0;
				const uint8_t* buffer = (uint8_t*)(handle->buffer);
				for (int stripe = 7; stripe >= 0; stripe--)
				{
					for (int row = 0; row < TI80_ROWS; row++)
					{
...
static int		get_memfree	(CalcHandle* handle, uint32_t* ram, uint32_t* flash)
{
	...
	*flash = -1;
	...
	int ret = SEND_REQ(handle, 0x0000, (handle->model == CALC_TI83) ? TI83_DIR : TI86_DIR, "\0\0\0\0\0\0\0");
	if (!ret)
	{
		ret = RECV_ACK(handle, &unused);
		if (!ret)
...
{
	int ret;
	static CalcLabEquipmentData lab_equipment_data_1 = { CALC_LAB_EQUIPMENT_DATA_TYPE_STRING, 4, 1, (const uint8_t *)"{7}", nullptr, TI89_LIST, 0, 0 }; // Request status command.
	CalcLabEquipmentData lab_equipment_data_2 = { .vartype = TI89_LIST };

	// Use the TI-68k format because it's easier to deal with.
	ret = tixx_send_lab_equipment_data_legacy(handle, CALC_TI89, &lab_equipment_data_1);
	if (!ret)
	{
		ret = tixx_get_lab_equipment_data_legacy(handle, CALC_TI89, &lab_equipment_data_2);
		if (!ret)
		{
			...
			if (lab_equipment_data_2.type == CALC_LAB_EQUIPMENT_DATA_TYPE_TI68K_RAW_LIST)
			{
				...
				ret = ticalcs_convert_lab_equipment_data_to_string(&lab_equipment_data_2, &item_count, &raw_values, &string_data);
				if (!ret)
				{
					if (item_count >= 3)
					{
						InfosMask infos_mask = 0;
						const unsigned long device_code = (unsigned long)raw_values[0];
						...
						if (infos->model == CALC_CBR || infos->model == CALC_CBR2 || infos->model == CALC_CBL2 || infos->model == CALC_LABPRO)
						{
							...
							if (item_count >= 17)
							{
								sprintf(infos->user_defined_id, "%10.10Lg", raw_values[16]);
		...
		ticalcs_calc_free_lab_equipment_data(&lab_equipment_data_2);
	}
