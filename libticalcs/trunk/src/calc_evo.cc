...
static int evo_dir_entry_matches_request(const EvoCborValue *item, const VarRequest *vr)
{
	...
	if (!g_ascii_strcasecmp(display, vr->name))
	{
		return 1;
	}
	if (type->v.uintv == EVO_TYPE_MATRIX)
	{
		...
		if (len >= 2 && display[0] == '[' && display[len - 1] == ']')
		{
			...
			memcpy(bare, display + 1, bare_len);
			bare[bare_len] = 0;
	...
	return type->v.uintv == EVO_TYPE_TABLE_SETUP && !g_ascii_strcasecmp(display, "tblsetup") && !g_ascii_strcasecmp(vr->name, "tblset");
}
...
static int send_var(CalcHandle *handle, CalcMode mode, FileContent *content)
{
	...
	for (unsigned int i = 0; i < content->num_entries; i++)
	{
		...
		const unsigned int archive = ve->attr == ATTRB_ARCHIVED || ve->type == EVO_TYPE_PICTURE || ve->type == EVO_TYPE_IMAGE;
		char url[96];
