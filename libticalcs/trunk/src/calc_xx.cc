...
int TICALL ticalcs_calc_control_lab_equipment(CalcHandle *handle, CalcModel model, CalcLabEquipmentParameters * params)
{
	...
	{
		CalcEventData event;
		ticalcs_event_fill_header(handle, &event, /* type */ CALC_EVENT_TYPE_BEFORE_GENERIC_OPERATION, /* retval */ 0, /* operation */ FNCT_CONTROL_LAB_EQUIPMENT);
		event.model = model;
		memset((void *)&event.data, 0, sizeof(event.data));
		ret = ticalcs_event_send(handle, &event);
		if (!ret)
		...
		ticalcs_event_fill_header(handle, &event, /* type */ CALC_EVENT_TYPE_AFTER_GENERIC_OPERATION, /* retval */ ret, /* operation */ FNCT_CONTROL_LAB_EQUIPMENT);
		event.model = model;
		memset((void *)&event.data, 0, sizeof(event.data));
		ret = ticalcs_event_send(handle, &event);
	}
