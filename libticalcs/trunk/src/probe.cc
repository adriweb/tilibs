...
CalcModel TICALL ticalcs_device_info_to_model(const CableDeviceInfo *info)
{
	...
	if (info != nullptr)
	{
		...
		else if (info->family == CABLE_FAMILY_USB_TI8X)
		{
			...
			else if (info->variant == CABLE_VARIANT_LABPRO)
			{
				model = CALC_LABPRO_USB;
			}
			else if (info->variant == CABLE_VARIANT_EASYTEMP_GOTEMP)
			{
				model = CALC_EASYTEMP_GOTEMP_USB;
			}
			else if (info->variant == CABLE_VARIANT_EASYLINK_GOLINK)
			{
				model = CALC_EASYLINK_GOLINK_USB;
			}
			else if (info->variant == CABLE_VARIANT_CBR2_GOMOTION)
			{
				model = CALC_CBR2_GOMOTION_USB;
			}
			else if (info->variant == CABLE_VARIANT_GODIRECT)
			{
