...
#include "../src/calclabequipmentdata.h"
#include "../src/error.h"
...
static int control_lab_equipment(CalcHandle * h, int, char * input)
{
	int model;
	double period;
	int ret;
	...
	CalcLabEquipmentParameters params;

	printf("Enter calc model, measurement period (s, 0=default), led_color (0xFF=unchanged), led_brightness (0xFF=unchanged), analog_input_channel (0xFF=unchanged), start_measurements, stop_measurements, reset, sensor_mask: ");
	ret = 0;
	if (NULL != input && input[0] != 0)
	{
		ret = sscanf(input, "%d %lf %hhu %hhu %hhu %hhu %hhu %hhu %u", &model, &period, &led_color, &led_brightness, &analog_input_channel, &start_measurements, &stop_measurements, &reset, &sensor_mask);
		if (!isatty(0))
		{
			printf("%d %lf %u %u %u %u %u %u %u\n", model, period, led_color, led_brightness, analog_input_channel, start_measurements, stop_measurements, reset, sensor_mask);
		}
	}
	if (ret < 9)
	{
		ret = scanf("%d %lf %hhu %hhu %hhu %hhu %hhu %hhu %u", &model, &period, &led_color, &led_brightness, &analog_input_channel, &start_measurements, &stop_measurements, &reset, &sensor_mask);
		if (!isatty(0))
		{
			printf("%d %lf %u %u %u %u %u %u %u\n", model, period, led_color, led_brightness, analog_input_channel, start_measurements, stop_measurements, reset, sensor_mask);
		}
	...
	params.led_brightness = led_brightness;
	params.analog_input_channel = analog_input_channel;
	params.start_measurements = start_measurements;
	params.stop_measurements = stop_measurements;
	params.reset = reset;
	params.sensor_mask = sensor_mask;

	ret = ticalcs_calc_control_lab_equipment(h, (CalcModel)model, &params);
...
static int send_lab_equipment_data(CalcHandle * h, int, char * input)
{
	...
	printf("Enter calc model, variable type (4 for TI-68k lists, 1 for TI-73/82/83/83+/84+ lists, 4 for TI-85/86 lists) and data type (1=string, 2=TI-68k raw, 3=TI-85/86 raw, 4=TI-Z80 raw): ");
	int ret = scan_print_output_3(input, "%d %u %u", "%d %u %u", &model, &vartype, &datatype, model, vartype, datatype);
	if (ret < 3)
	{
		fputs("Missing parameters\n", stderr);
		return 1;
	...
	if (nullptr != ptr)
	{
		ticalcs_fill_lab_equipment_data(&lab_equipment_data, CALC_LAB_EQUIPMENT_DATA_TYPE_STRING,
		                                (uint16_t)(strlen((const char *)ptr) + 1), 0, ptr, nullptr, (uint8_t)vartype, 0, 0);

		// Send the data in the format selected by the caller. A STRING data type is passed through
		// as-is (the library picks the wire format from the model); every other data type exercises
		// the matching string->raw conversion and sends the converted result.
		CalcLabEquipmentData * to_send = &lab_equipment_data;
		CalcLabEquipmentData selected;
		if (datatype != CALC_LAB_EQUIPMENT_DATA_TYPE_STRING)
		{
			ticalcs_fill_lab_equipment_data(&selected, (CalcLabEquipmentDataType)datatype, 0, 0, nullptr, nullptr, (uint8_t)vartype, 0, 0);
			ret = ticalcs_convert_lab_equipment_data_from_string(&selected, (const char *)ptr);
			if (ret)
			{
				printf("Conversion to selected raw list failed (%d)\n", ret);
				ticalcs_calc_free_lab_equipment_data(&selected);
				return ret;
			}
			printf("Raw data (%u bytes): ", selected.size);
			for (uint16_t j = 0; j < selected.size; j++)
			{
				printf("%02X ", selected.data[j]);
			}
			putchar('\n');
			to_send = &selected;
		}

		ret = ticalcs_calc_send_lab_equipment_data(h, (CalcModel)model, to_send);
...
static int get_lab_equipment_data(CalcHandle * h, int, char * input)
{
	...
	printf("Enter calc model, variable type (4 for TI-68k lists, 1 for TI-73/82/83/83+/84+ lists, 4 for TI-85/86 lists) and expected data type (1=string, 2=TI-68k raw, 3=TI-85/86 raw, 4=TI-Z80 raw): ");
	int ret = scan_print_output_3(input, "%d %u %u", "%d %u %u", &model, &vartype, &datatype, model, vartype, datatype);
	if (ret < 3)
	{
		fputs("Missing parameters\n", stderr);
	...
	if (!ret)
	{
		...
		if (datatype != lab_equipment_data.type)
		{
			printf("Note: expected data type %u, got %u\n", datatype, lab_equipment_data.type);
		}
		...
		char * str = nullptr;
		int cret = ticalcs_convert_lab_equipment_data_to_string(&lab_equipment_data, &item_count, &raw_values, &str);
		if (!cret)
		{
			printf("Converted (%u items): %s\n", item_count, str);
		...
		{
			printf("String conversion failed (%d)\n", cret);
		}
		ticalcs_free_lab_equipment_data_related(str);
		free(raw_values);
	}
...
static int send_lab_equipment_data_raw(CalcHandle * h, int, char * input)
{
	int ret;
	CalcLabEquipmentData lab_equipment_data;
	uint8_t raw[8];
	...
	printf("Enter calc model (usually 1-12, 16-17) and number of raw bytes (8 for Go! Temp / Go! Link): ");
	ret = scan_print_output_2(input, "%d %u", "%d %u", &model, &nbytes, model, nbytes);
	if (ret < 2 || nbytes > sizeof(raw))
	{
		fputs("Missing or invalid parameters\n", stderr);
		return 1;
	}

	printf("Enter raw data as hex bytes: ");
	ret = 1;
	{
		char * ptr = (char *)fgets((char *)inbuf + 1024 + 4, 1024, stdin);
		if (NULL != ptr)
		{
			unsigned int i;
			char * endptr;
			...
			for (i = 0; i < nbytes; i++)
			{
				raw[i] = (uint8_t)strtoul(ptr, &endptr, 16);
				if (endptr == ptr)
				{
			...
			if (!ret)
			{
				lab_equipment_data.type = CALC_LAB_EQUIPMENT_DATA_TYPE_RAW;
				lab_equipment_data.size = (uint16_t)nbytes;
				lab_equipment_data.items = 0;
				lab_equipment_data.data = raw;
				lab_equipment_data.vartype = 0;
				lab_equipment_data.varname = NULL;
...
static int get_lab_equipment_data_raw(CalcHandle * h, int, char * input)
{
	int ret;
	CalcLabEquipmentData lab_equipment_data;
	int model;
	...
	printf("Enter calc model (usually 1-12, 16-17) and expected number of raw bytes (8 for Go! Temp / Go! Link): ");
	ret = scan_print_output_2(input, "%d %u", "%d %u", &model, &nbytes, model, nbytes);
	if (ret < 2)
	{
		fputs("Missing parameters\n", stderr);
	...
	if (!ret)
	{
		printf("Received raw data type %d size %" PRIu16 " items %" PRIu16 "\n", lab_equipment_data.type, lab_equipment_data.size, lab_equipment_data.items);
		const uint8_t * ptr = lab_equipment_data.data;
		for (uint16_t i = 0; i < lab_equipment_data.size; i++)
		{
			printf("%02X ", ptr[i]);
