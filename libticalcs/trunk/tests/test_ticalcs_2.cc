...
static int send_lab_equipment_data(CalcHandle * h, int, char * input)
{
	...
	if (nullptr != ptr)
	{
		...
		lab_equipment_data.size = (uint16_t)strlen((const char *)ptr) + 1U;
...
#define NULL_ENTRY \
	...
	NULL_ENTRY,
	DEFINE_MENU_ENTRY("FNCT Send({...}) (send list data)", send_lab_equipment_data),           // 40
	...
	DEFINE_MENU_ENTRY("BUF  Poke data in the packet buffer", buffer_poke_data),                // 45
	...
	NULL_ENTRY,
	DEFINE_MENU_ENTRY("BUF  AND register/value to register", reg_and),                         // 50
	...
	DEFINE_MENU_ENTRY("RAW  Recv raw data bytes", raw_recv_data),                              // 55
	NULL_ENTRY,
	...
	NULL_ENTRY,
	DEFINE_MENU_ENTRY("DUSB Send raw packet", dusb_send_rpkt),                                 // 60
	...
	DEFINE_MENU_ENTRY("RDMP Check whether ready", rdmp_is_ready),                              // 65
	...
	DEFINE_MENU_ENTRY("DBUS 83+ Memory dump", ti83p_dump),                                     // 70
	NULL_ENTRY,
	...
	NULL_ENTRY,
	DEFINE_MENU_ENTRY("DBUS 83+ Get standard calculator ID", ti83p_gid),                       // 75
	...
	NULL_ENTRY,
	DEFINE_MENU_ENTRY("NSP  Send key (specific and generic)", nsp_send_key),                   // 80
	NULL_ENTRY,
