...
int TICALL ticonv_model_uses_utf8(CalcModel model)
{
	...
	return (   model != CALC_NONE
	        ...
	        && model != CALC_EASYTEMP_GOTEMP_USB
	        && model != CALC_EASYLINK_GOLINK_USB
	        && model != CALC_CBR2_GOMOTION_USB
	        && model != CALC_GODIRECT_USB);
}
...
int TICALL ticonv_model_has_usb_ioport(CalcModel model)
{
	return (   /*model <  CALC_MAX
	        &&*/ ( model == CALC_TI84P_USB
	            ...
	            || model == CALC_EASYLINK_GOLINK_USB
	            || model == CALC_CBR2_GOMOTION_USB
	            || model == CALC_GODIRECT_USB)); // NOTE: the LabPro's USB port is B, not mini-B, and it's known to speak a protocol other than DUSB/CARS on that interface, at least in some conditions.
}
...
int TICALL ticonv_model_has_flash_memory(CalcModel model)
{
	...
	return (   model != CALC_NONE
	        ...
	        && model != CALC_EASYLINK_GOLINK_USB
	        && model != CALC_CBR2_GOMOTION_USB
	        && model != CALC_GODIRECT_USB); // FIXME this might be wrong, but while we're aware of OS upgrades for CBL2, LabPro and TI-Presenter, we've never seen any for the CBR2.
}
...
CalcProductIDs TICALL ticonv_model_to_product_id(CalcModel model)
{
	switch (model)
	{
		...
		case CALC_NSPIRE:              return PRODUCT_ID_NSPIRE_CAS; // Generic Nspire can correspond to multiple models
		...
		case CALC_LABPRO_USB:          return PRODUCT_ID_NONE; // These devices don't speak DUSB/CARS, so the product ID is meaningless.
		case CALC_EASYTEMP_GOTEMP_USB: return PRODUCT_ID_NONE;
