	...
	if (filename != nullptr)
	{
...

		{
			...
			if (   !g_ascii_strcasecmp(e, "tns")
			    || tifiles_fext_is_evo(e)
			    || tifiles_fext_is_evo_os(e))
			{
				return !0;
			}
		}
...
int TICALL tifiles_file_is_single(const char *filename)
{
	...
	ret = (buffer_is_ti(&r) || !g_ascii_strcasecmp(e, "tns") || tifiles_fext_is_evo(e))
...
int TICALL tifiles_file_is_os(const char *filename)
{
	...
	if (!open_for_tifl(&r, filename))
	{
		...
		ret = ret || tifiles_fext_is_evo_os(tifiles_fext_get(filename));
		tifiles_mem_reader_destroy(&r);
	}
...
int TICALL tifiles_file_is_flash(const char *filename)
{
	...
	return ret || tifiles_fext_is_evo_os(tifiles_fext_get(filename));
}
