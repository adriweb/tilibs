...
static int compare_files(const char *src, const char *dst, int (* skip)(int offset))
{
	int i;
	int s, d;
	...
	{
		if ((s = fgetc(fs)) != (d = fgetc(fd)) && !skip(ftell(fs)))
		{
                                    ...
                                    const char * output_file)
{
	...
	if (content != nullptr)
	{
		...
		if (!ret)
		{
			...
			if (!ret)
			{
				ret = compare_files(input_file, output_file, skip);
			}
			else
			{
				printf("tifiles_file_write_backup: %i\n", ret);
			}
			tifiles_content_delete_backup(content);
		}
		else
		{
			printf("tifiles_file_read_backup: %i\n", ret);
		}
	}
                                           ...
                                           const char * output_group)
{
	...
	if (content != nullptr)
	{
		...
		if (!ret)
		{
			...
			if (!ret)
			{
				compare_files(input_group, output_group, skip);
			}
			tifiles_filename_free(unused);
			tifiles_content_delete_regular(content);
		}
                                   ...
                                   const char * output_group_file)
{
	...
	if (!ret)
	{
		...
		ret = compare_files(input_group_file, output_group_file, skip);
	}
                                     ...
                                     const char * input_file_4)
{
	...
	if (!ret)
	{
		...
		ret = compare_files(dest_file_1, input_file_3, skip);
		if (!ret)
		{
			ret = compare_files(dest_file_2, input_file_4, skip);
		}
	}
...
/* TI-8x */
/*********/
static int skip_ti8x_comments(int offset)
{
	// Comment offsets for regular/group and backup files.
	return (offset >= 0xB && offset <= 0x35);
}

static int skip_ti9x_comments(int offset)
{
	// Comment offsets for regular/group and backup files.
	return (offset >= 0x12 && offset < 0x3A);
}

static int skip_tig_comments(int offset)
{
	// Comment offsets for tigroup files.
	return 1;
}
...
static int test_ti8x_group_merge()
{
	...
	if (!ret)
	{
		...
		if (!ret)
		{
			...
			if (!ret)
			{
				ret = compare_files("misc/group1.8Xg", "misc/group2.8Xg", skip_ti8x_comments);
			}
		}
	}
	...
	if (!ret)
	{
		...
		if (content != nullptr)
		{
			...
			if (!ret)
			{
				...
				if (!ret)
				{
					ret = compare_files("tig/test.tig", "tig/test_.tig", skip_tig_comments);
				}
			}
			tifiles_content_delete_tigroup(content);
		}
			...
			if (!ret)
			{
				...
				if (!ret)
				{
					...
					if (!ret)
					{
						...
						ret = compare_files("tig/test.tig", "tig/test2.tig", skip_tig_comments);
					}
				}
			}
		}
	...
	if (!ret)
	{
		...
		if (!ret)
		{
			...
			if (!ret)
			{
				...
				ret = compare_files("tig/A.8Xn", "tig/AA.8Xn", skip_ti8x_comments);
				if (!ret)
				{
					ret = compare_files("tig/B.8Xn", "tig/BB.8Xn", skip_ti8x_comments);
				}
			}
		}
	}
