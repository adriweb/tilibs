...
// Keep protocol text as byte strings: each code unit is the original HP byte.
// TextDecoder('latin1') aliases Windows-1252 in browsers and changes 0x80-0x9F;
// UTF-8 also cannot round-trip names in the calculator's character set.
function latin1Bytes(value, label = 'HP RHOST commands') {
    const text = String(value);
    const output = new Uint8Array(text.length);
    for (let index = 0; index < text.length; index += 1) {
        const code = text.charCodeAt(index);
        if (code > 0xFF) {
            throw new Error(`${label} must use the calculator's single-byte character encoding.`);
        }
        output[index] = code;
    }
    return output;
...
class KermitClient {
    ...
    async readPacket(checksumSize = this.parameters.checksumSize) {
        ...
        if (packet.type === 'E') {
            const message = latin1Text(decodeData(packet.data, this.parameters));
    ...
    async receiveTransfer(firstPacket = null, initialChecksumSize = 1) {
        ...
        while (true) {
            ...
            if (!sawHeader && (packet.type === 'F' || packet.type === 'X')) {
                remoteName = latin1Text(decodeData(packet.data, this.parameters));
                sawHeader = true;
                await this.transport.write(this.packet(expectedSeq, 'Y'));
            ...
            if (sawHeader && packet.type === 'Z') {
                await this.transport.write(this.packet(expectedSeq, 'Y'));
                expectedSeq = (expectedSeq + 1) & 0x3F;
                // Keep reading through the duplicate-packet handler: a lost
                // EOF acknowledgement makes the sender retransmit Z.
                sawEof = true;
                continue;
            }
    ...
    async listDirectory() {
        return this.withOperation(async () => {
            ...
            return latin1Text(transfer.data);
        });
    }
    ...
    async hostCommand(command) {
        return this.withOperation(async () => {
            ...
            if (firstPacket.type === 'Y' && firstPacket.seq === 0) {
                return latin1Text(decodeData(firstPacket.data, this.parameters));
            }
    ...
    async receiveFile(name) {
        return this.withOperation(async () => {
            const nameBytes = latin1Bytes(name, 'HP filenames');
            this.resetParameters();
            await this.waitForServerReady();
            const encodedName = encodeData(nameBytes, this.parameters);
            const firstPacket = await this.requestReceiveStart(
...
function parseHpRdir(text) {
    ...
    // RDIR uses ASCII spacing. Bytes such as 0xA0 are HP name characters,
    // even though JavaScript's Unicode-aware \s and trim() treat them as space.
    const trimSpacing = value => value.replace(/^[ \t]+|[ \t]+$/g, '');
    const directoryMatch = /\{[ \t]*(.*?)[ \t]*\}[ \t]*([0-9.]+)/s.exec(normalized);
    if (!directoryMatch) throw new Error('The HP directory response has no { PATH } header.');
    const pathParts = trimSpacing(directoryMatch[1]).split(/[ \t]+/).filter(Boolean);
    ...
    const entryPattern = /([^ \t][\s\S]*?)[ \t]+([0-9.]+)[ \t]+(.+?)[ \t]+([0-9.]+)(?=[ \t]+[^ \t]|[ \t]*$)/g;
    const entries = [];
    for (const match of body.matchAll(entryPattern)) {
        const name = trimSpacing(match[1]);
        const typeName = trimSpacing(match[3]);
