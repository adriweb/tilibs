...
function testRdirPreservesExtendedNameBytes() {
    const names = Array.from({ length: 128 }, (_, index) =>
        `${String.fromCharCode(0x80 + index)}NAME${String.fromCharCode(0x80 + index)}`);
    const parsed = parseHpRdir(`{ HOME \xA0FOLDER\xA0 } 205651.\n`
        + names.map(name => `${name} 26. Real Number 13207.`).join('\n'));
    assert.deepEqual(parsed.pathParts, ['HOME', '\xA0FOLDER\xA0']);
    assert.deepEqual(parsed.entries.map(entry => entry.name), names,
        'all native HP high bytes must survive at both ends of a name');
    assert.ok(parsed.entries.every(entry => entry.folder === '\xA0FOLDER\xA0'));
}
...
async function testDirectoryGenericCommandAndInboundTransfer() {
    const listing = Uint8Array.from(Buffer.from(
        '{ HOME } 205642.\n\x80TEST\xA0 57. Directory 59130.\n\x9FIOPAR\xFF 29.5 List 45664.', 'latin1'));
    let step = 0;
    ...
    assert.deepEqual(parsed.entries.map(entry => entry.name), ['\x80TEST\xA0', '\x9FIOPAR\xFF']);
}
...
async function testFilenameBytesRoundTrip() {
    const negotiated = applyNegotiatedParameters(parameterRequest(3));
    const payload = Uint8Array.of(0, 0x80, 0xFF);
    for (let highByte = 0x80; highByte <= 0xFF; highByte += 1) {
        const nameBytes = Uint8Array.of(highByte, 0x4E, highByte);
        const name = Buffer.from(nameBytes).toString('latin1');
        let step = 0;
        let client;
        const transport = new ScriptedTransport(async (wire, mock) => {
            const packet = parsePacket(packetWithoutTerminator(wire),
                step <= 1 || step === 6 ? 1 : 3);
            assert.equal(packet.checksumValid, true);
            if (step === 0) {
                assert.equal(packet.type, 'R');
                assert.deepEqual(decodeData(packet.data, client.parameters), nameBytes,
                    'GET must request the original single-byte name');
                mock.enqueue(makePacket(0, 'S', parameterRequest(3), 1));
            } else if (step === 1) {
                mock.enqueue(makePacket(1, 'F', encodeData(nameBytes, negotiated), 3));
            } else if (step === 2) {
                mock.enqueue(makePacket(2, 'D', encodeData(payload, negotiated), 3));
            } else if (step === 3) {
                mock.enqueue(makePacket(3, 'Z', [], 3));
                ...
                assert.equal(packet.type, 'S');
                mock.enqueue(makePacket(0, 'Y', HP50_PARAMETER_ACK, 1));
            } else if (step >= 7) {
                if (step === 7) {
                    assert.equal(packet.type, 'F');
                    assert.deepEqual(decodeData(packet.data, client.parameters), nameBytes,
                        'sending a received filename must preserve every original byte');
                }
                mock.enqueue(makePacket(packet.seq, 'Y', [], 3));
            }
            step += 1;
        });
        transport.enqueue(makePacket(0, 'N', [], 1));
        client = new KermitClient(transport, { timeoutMs: 50, retries: 2 });
        const received = await client.receiveFile(name);
        assert.equal(received.name, name,
            'incoming F packets must not remap 0x80-0x9F through Windows-1252');
        assert.deepEqual(received.data, payload);
        transport.enqueue(makePacket(0, 'N', [], 1));
        await client.sendFile(received.name, received.data);
        assert.equal(step, 11);
    }
}

async function testUnsupportedFilenameRejectedBeforeTransaction() {
    const transport = new ScriptedTransport(() => assert.fail('no packet may be written'));
    const client = new KermitClient(transport);
    await assert.rejects(client.receiveFile('ΣDAT'), /HP filenames.*single-byte/);
    await assert.rejects(client.sendFile('ΣDAT', Uint8Array.of(1)), /HP filenames.*single-byte/);
    assert.equal(transport.readCount, 0);
    assert.equal(client.busy, false);
}

async function testReceiveAcknowledgesRetransmittedEof() {
    const negotiated = applyNegotiatedParameters(parameterRequest(3));
    for (const dataPacketCount of [1, 61, 62]) {
        const eofSeq = (dataPacketCount + 2) & 0x3F;
        const endSeq = (eofSeq + 1) & 0x3F;
        const replies = [makePacket(1, 'F', encodeData(Buffer.from('TEST'), negotiated), 3)];
        for (let index = 0; index < dataPacketCount; index += 1) {
            replies.push(makePacket((index + 2) & 0x3F, 'D',
                encodeData(Uint8Array.of(index), negotiated), 3));
        }
        // Lose two consecutive EOF ACKs, including across the 63 -> 0 wrap.
        replies.push(makePacket(eofSeq, 'Z', [], 3), makePacket(eofSeq, 'Z', [], 3),
            makePacket(eofSeq, 'Z', [], 3), makePacket(endSeq, 'B', [], 3));
        const acknowledgements = [];
        const transport = new ScriptedTransport(async (wire, mock) => {
            const packet = parsePacket(packetWithoutTerminator(wire),
                acknowledgements.length === 0 ? 1 : 3);
            assert.equal(packet.checksumValid, true);
            assert.equal(packet.type, 'Y');
            acknowledgements.push(packet.seq);
            if (replies.length) mock.enqueue(replies.shift());
        });
        const client = new KermitClient(transport);
        const received = await client.receiveTransfer(parsePacket(
            packetWithoutTerminator(makePacket(0, 'S', parameterRequest(3), 1)), 1));
        assert.equal(received.name, 'TEST');
        assert.deepEqual(received.data, Uint8Array.from({ length: dataPacketCount }, (_, index) => index));
        assert.deepEqual(acknowledgements.slice(-4), [eofSeq, eofSeq, eofSeq, endSeq]);
...
(async () => {
    ...
    testRdirPreservesExtendedNameBytes();
    ...
    await testFilenameBytesRoundTrip();
    await testUnsupportedFilenameRejectedBeforeTransaction();
    await testReceiveAcknowledgesRetransmittedEof();
