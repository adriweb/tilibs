...
async function testAppProgramUploadTargeting() {
    const apps = ['Gallery', 'Other'].map((name, hpIndex) => ({ name, hpIndex, hpAppRoot: true }));
    const state = {
        hpFileSnapshotLoaded: true, expandedFolders: new Set(),
        dirlist: [...apps, ...apps.flatMap(app => ['hpappprgm', 'hpappnote'].map(extension => ({
            name: `${app.name}.${extension}`, kind: 'hp-app-child', hpIndex: app.hpIndex,
            hpAppChildEditable: false
        })))]
    };
    let answer = 'Other', approve = true, updates = [], calls = [], logs = [], prompts = [];
    const module = { FS: { writeFile() {}, unlink() {} } };
    const context = {
        state, Uint8Array, console,
        t: key => key, tFormat: (key, args) => key + JSON.stringify(args),
        log: message => logs.push(message),
        prompt: (message, suggested) => { prompts.push([message, suggested]); return answer; },
        confirm: () => approve,
        initModule: async () => module,
        encodeHPPrimeAppResourceManifest: entries => { updates = entries; return new Uint8Array(); },
        ccallAsync: async (_module, name, _type, _argTypes, args) => { calls.push([name, args]); return 0; },
        getEntryFolderPath: app => app.name,
        applyHPPrimeFileSnapshot() {},
        setButtonLoading() {},
        updateSendFilesButtonState() {},
        isHPPrimeActive: () => true,
        isHPLegacyActive: () => false,
        isCasioActive: () => false,
        els: { fileInput: { files: [] }, btnSendFiles: {} },
        setSelectedFiles: files => { state.selectedFiles = files; }
    };
    vm.createContext(context);
    for (const name of ['chooseHPPrimeAppForFile', 'sendHPPrimeAppResources', 'sendSelectedFiles']) {
        vm.runInContext(extractFunction(name), context);
    }
    const file = { name: 'Gallery.hpappprgm', arrayBuffer: async () => Uint8Array.of(1, 2, 3, 4).buffer };
    assert.equal(context.chooseHPPrimeAppForFile(file), apps[1]);
    assert.equal(prompts[0][1], 'Gallery', 'suggest same-name app but allow a different target');
    assert.equal(await context.sendHPPrimeAppResources([file], apps[1]), true);
    assert.equal(updates[0].name, 'Other.hpappprgm', 'target the app core section, not a resource named after the source');
    assert.deepEqual(Array.from(updates[0].data), [1, 2, 3, 4], 'native adapter handles source conversion');
    assert.equal(calls[0][0], 'hp_prime_send_cached_app_resources');
    assert.equal(calls[0][1][0], 1);
    approve = false;
    await context.sendHPPrimeAppResources([file], apps[1]);
    assert.equal(calls.length, 1, 'keep overwrite confirmation for core parts');
    approve = true;
    await context.sendHPPrimeAppResources([{ ...file, name: 'Gallery.hpapp' }], apps[1]);
    assert.equal(calls.length, 1, 'do not upload a descriptor as a resource');
    answer = null;
    assert.equal(context.chooseHPPrimeAppForFile(file), null, 'cancel never chooses an app implicitly');
    answer = 'missing';
    assert.equal(context.chooseHPPrimeAppForFile(file), null);
    assert.match(logs.at(-1), /hp_prime_app_target_missing/);
    state.hpFileSnapshotLoaded = false;
    assert.equal(context.chooseHPPrimeAppForFile(file), null);
    await context.sendHPPrimeAppResources([file], apps[1]);
    assert.equal(calls.length, 1, 'no update against a partial or stale snapshot');
    state.hpFileSnapshotLoaded = true;
    answer = 'Gallery';
    state.selectedFiles = [file];
    await context.sendSelectedFiles();
    assert.equal(calls.length, 2);
    assert.equal(calls[1][0], 'hp_prime_send_cached_app_resources', 'standalone program is never sent as a top-level variable');
    assert.equal(updates[0].name, 'Gallery.hpappprgm');
    assert.equal(state.selectedFiles.length, 0);
}

Promise.all([
    testAppProgramUploadTargeting(),
